...
def main():
    ...
    for knowledge in tqdm(knowns):
        ...
        if "llama" in mt.model_name.lower():
            kinds = [None, "mlp", "self_attn"]
        else:
            kinds = [None, "mlp", "attn"]
        for kind in kinds:
            ...
            if known_id > 200:
                continue
            plot_trace_heatmap(plot_result, savepdf=pdfname, modelname=mt.model_name)
    ...
    def __init__(
        ...
        if model is None:
            ...
            model = AutoModelForCausalLM.from_pretrained(
                model_path+model_name, low_cpu_mem_usage=low_cpu_mem_usage, torch_dtype=torch_dtype
            )
...
def plot_hidden_flow(
    ...
    plot_trace_heatmap(result, savepdf,mt.model_name)
...
def predict_token(mt, prompts, return_p=False):
    ...
    result = [mt.tokenizer.decode(c, skip_special_tokens=True) for c in preds]
    if return_p:
        result = (result, p)
    return result
