...
def main(
    ...
    if any(alg in alg_name for alg in ["AlphaEdit","MEMIT_prune", "NSE"]):
    ...
    for record_chunks in chunks(ds, num_edits):
        ...
        etc_args = dict(cache_template=cache_template) if any(alg in alg_name for alg in ["ROME", "MEMIT","AlphaEdit", "MEMIT_prune", "NSE"]) else dict()
        ...
        start = time()
        if any(alg in alg_name for alg in ["AlphaEdit", "NSE"]):
        ...
        post_edit_hs_path = Path(f"./hs/post_batch_{num_edits}_edit_hs_{alg_name}_{'blue' if hparams.blue else ''}_{model_name.replace('/', '_')}_{ds_name}.pt")
        hs = get_module_input_output_at_words(
                edited_model,
                tok,
                hparams.layers[-1],
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--alg_name",
        choices=["AlphaEdit", "MEMIT_rect","MEMIT_prune", "MEMIT", "ROME", "FT", "MEND","NSE","PMET"],
        default="AlphaEdit",
