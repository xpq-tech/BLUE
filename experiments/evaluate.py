...
from pmet import PMETHyperParams, apply_pmet_to_model, apply_pmet_seq_to_model
...
ALG_DICT = {
    ...
    "PMET_seq": (PMETHyperParams, apply_pmet_seq_to_model),
}
...
def main(
    ...
    if use_cache:
        if any(alg in alg_name for alg in ["MEMIT","AlphaEdit", "MEMIT_seq", "MEMIT_prune", "MEMIT_rect"]):
            cache_template = (
                KV_DIR
                / f"{model_name.replace('/', '_')}_MEMIT{'_BLUE' if hparams.blue else ''}"
                / f"{ds_name}_layer_{{}}_clamp_{{}}_case_{{}}.npz"
            )
        elif any (alg in alg_name for alg in ["PMET", "PMET_seq"]):
            cache_template = (
                KV_DIR
                / f"{model_name.replace('/', '_')}_{alg_name}{'_BLUE' if hparams.blue else ''}"
    ...
    if any(alg in alg_name for alg in ["AlphaEdit", "MEMIT_seq", "MEMIT_prune", "NSE", "PMET_seq"]):
        ...
        elif hparams.model_name in ["EleutherAI_gpt-j-6B","Llama3-8B", "Llama3-8B-Instruct"]:
    ...
    if alg_name == "AlphaEdit":
        null_space_project_save_dir = Path(DATA_DIR / "null_space_projects")
        null_space_project_save_dir.mkdir(exist_ok=True, parents=True)
        project_file = null_space_project_save_dir / f"{hparams.model_name}_layers_{hparams.layers[0]}-{hparams.layers[-1]}.pt"
        for i, layer in enumerate(hparams.layers):
            P[i,:,:] = get_project(model,tok,layer,hparams)
        torch.save(P, project_file)
    pre_edit_hs_path = Path(f"./hs/pre_edit_hs_{model_name.replace('/', '_')}.pt")
    if not pre_edit_hs_path.exists():
        ...
        torch.save(hs, pre_edit_hs_path)
        del hs
    ...
    for record_chunks in chunks(ds, num_edits):
        ...
        etc_args = dict(cache_template=cache_template) if any(alg in alg_name for alg in ["ROME", "MEMIT","AlphaEdit", "MEMIT_seq", "MEMIT_prune", "NSE", "PMET_seq"]) else dict()
        seq_args = dict(cache_c=cache_c) if any(alg in alg_name for alg in ["AlphaEdit", "MEMIT_seq", "NSE", "PMET_seq"]) else dict()
        nc_args = dict(P = P) if any(alg in alg_name for alg in ["AlphaEdit"]) else dict()

        start = time()
        if any(alg in alg_name for alg in ["AlphaEdit", "MEMIT_seq", "NSE", "PMET_seq"]):
        ...
        if args.downstream_eval_steps > 0 and cnt % args.downstream_eval_steps == 0:
            ...
            out_file = glue_save_location + "cnt_{}.json".format(cnt)#stores the last case ID of the batch
    ...
    post_edit_hs_path = Path(f"./hs/post_seq_{num_edits}_edit_hs_{alg_name}_{'blue' if hparams.blue else ''}_{model_name.replace('/', '_')}_{ds_name}.pt")
    hs = get_module_input_output_at_words(
            edited_model,
            tok,
            hparams.layers[-1],
    ...
    os.makedirs("./hs", exist_ok=True)
    torch.save(hs, post_edit_hs_path)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--alg_name",
        choices=["AlphaEdit", "PMET_seq", "MEMIT_rect", "MEMIT_seq","MEMIT_prune", "MEMIT", "ROME", "FT", "MEND","NSE","PMET"],
        default="AlphaEdit",
