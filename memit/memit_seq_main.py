...
def apply_memit_seq_to_model(
    ...
    if hparams.blue:
        deltas, cache_c = execute_memit_lbl(model, tok, requests, hparams, cache_template=cache_template, cache_c=cache_c)
    else:
...
def execute_memit_lbl(
    ...
    for i, layer in enumerate(hparams.layers):
        ...
        resid = targets  # Distribute residual across layers
        upd_matrix = resid @ adj_k.T
