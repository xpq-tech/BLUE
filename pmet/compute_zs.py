...
def get_target_new_avg_prob(
    ...
    input_tok = tok(
        [prompt.format(request["subject"]) for prompt in rewriting_prompts],
        return_tensors="pt",
        padding=True,
    ).to("cuda")
    ...
    return torch.exp(-nll_loss_each).mean().item()
